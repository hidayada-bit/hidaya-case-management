...
const ToastColors = {
  success: { bg:'#f0fdf4', border:'#86efac', color:'#14532d', icon:'✓' },
  error:   { bg:'#fef2f2', border:'#fca5a5', color:'#991b1b', icon:'✗' },
  info:    { bg:'#eff6ff', border:'#93c5fd', color:'#1e3a8a', icon:'ℹ' },
}
...
const NAV = [
  ...
  { id:'analytics', label:'Statistics', d:'M18 20V10 M12 20V4 M6 20v-6' },
...
function Sidebar({ page, setPage, collapsed, setCollapsed, user, onSignOut }) {
  return (
    <aside style={{ width:collapsed?66:200, flexShrink:0, background:'#0c1f3f', height:'100vh', position:'sticky', top:0, display:'flex', flexDirection:'column', transition:'width 0.22s ease', overflow:'hidden' }}>
      <div style={{ padding:collapsed?'16px 10px':'16px', borderBottom:'1px solid rgba(255,255,255,0.08)', display:'flex', alignItems:'center', gap:10, justifyContent:collapsed?'center':'flex-start' }}>
        <div style={{ width:38, height:38, borderRadius:10, background:'#fff', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, overflow:'hidden' }}>
          ...
          <div style={{ display:'none', width:34, height:34, alignItems:'center', justifyContent:'center', fontSize:18 }}>🕊</div>
        </div>
        {!collapsed && (
          <div>
            <div style={{ fontSize:12, fontWeight:800, color:'#fff', lineHeight:1.2 }}>Hidaya</div>
            <div style={{ fontSize:9, color:'#94a3b8', fontWeight:500 }}>Development Association</div>
          </div>
        )}
      </div>
      <nav style={{ flex:1, padding:'10px 6px' }}>
        {NAV.map(item=>(
          <button key={item.id} onClick={()=>setPage(item.id)} style={{ width:'100%', padding:collapsed?'11px':'10px 12px', borderRadius:9, border:'none', cursor:'pointer', background:page===item.id?'rgba(29,78,216,0.35)':'transparent', display:'flex', alignItems:'center', gap:10, marginBottom:3, justifyContent:collapsed?'center':'flex-start', transition:'background 0.15s' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={page===item.id?'#93c5fd':'#64748b'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d={item.d}/></svg>
      ...
      <div style={{ padding:collapsed?'10px 6px':'10px 12px', borderTop:'1px solid rgba(255,255,255,0.08)' }}>
        {!collapsed && user && (
          <div style={{ marginBottom:8 }}>
            <div style={{ fontSize:12, fontWeight:600, color:'#e2e8f0', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{user.full_name||user.email}</div>
            <div style={{ fontSize:10, color:'#64748b', textTransform:'capitalize' }}>{user.role||'staff'}</div>
          </div>
        )}
        <button onClick={onSignOut} style={{ width:'100%', padding:'7px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:8, cursor:'pointer', color:'#94a3b8', fontSize:12, display:'flex', alignItems:'center', justifyContent:collapsed?'center':'flex-start', gap:6 }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
          {!collapsed && 'Sign Out'}
        </button>
      </div>
      <button onClick={()=>setCollapsed(!collapsed)} style={{ margin:'0 6px 10px', padding:'7px', border:'1px solid rgba(255,255,255,0.1)', borderRadius:8, background:'transparent', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
...
const StatCard = ({ label, value, icon, bg, color, sub, loading }) => (
  <div style={{ background:'#fff', borderRadius:13, border:'1px solid #e8f0fe', padding:'18px 20px', display:'flex', alignItems:'center', gap:14, flex:1, minWidth:140 }}>
    ...
    <div style={{ flex:1 }}>
      ...
      {loading ? <Skeleton h={24} w={60}/> : <div style={{ fontSize:26, fontWeight:800, color:'#0f172a', lineHeight:1 }}>{value}</div>}
...
// ─── DASHBOARD PAGE ───────────────────────────────────────────────────────────
function DashboardPage({ families, children, loading, onView, onAdd }) {
  ...
  return (
    <div style={{ padding:'22px 26px', maxWidth:1140, margin:'0 auto' }}>
      ...
      <div style={{ display:'flex', gap:14, marginBottom:20, flexWrap:'wrap' }}>
        <StatCard label="Total Families" value={families.length} icon="👨‍👩‍👧‍👦" bg="#dbeafe" color="#1d4ed8" sub={`${active} active`} loading={loading}/>
        <StatCard label="Total Children" value={children.length} icon="👶" bg="#d1fae5" color="#059669" sub="enrolled" loading={loading}/>
        <StatCard label="Pending Cases" value={pending} icon="⏳" bg="#fef3c7" color="#d97706" sub="need review" loading={loading}/>
        ...
        <div style={{ padding:'16px 20px', borderBottom:'1px solid #f1f5f9', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
            <div style={{ position:'relative' }}>
              ...
              <input value={search} onChange={e=>{setSearch(e.target.value);setPg(1)}} placeholder="Search name, code, phone…"
                style={{ paddingLeft:30, paddingRight:10, paddingTop:7, paddingBottom:7, border:'1.5px solid #e2e8f0', borderRadius:8, fontSize:13, outline:'none', fontFamily:'inherit', width:200 }}/>
            </div>
            ...
            <tbody>
              ...
              {!loading && paged.map(f=>(
                <tr key={f.id} style={{ borderBottom:'1px solid #f8fafc', cursor:'pointer' }}
                  ...
                  onClick={()=>onView(f)}>
                  <td style={{ padding:'12px 14px', fontWeight:700, color:'#1d4ed8', fontSize:13 }}>{f.family_code}</td>
                  <td style={{ padding:'12px 14px', fontSize:13, color:'#64748b' }}>{f.roll_number}</td>
                  ...
                  <td style={{ padding:'12px 14px', fontSize:13, color:'#475569' }}>{f.phone_number}</td>
                  <td style={{ padding:'12px 14px', fontSize:13, color:'#475569' }}>{f.district}</td>
                  <td style={{ padding:'12px 14px' }}>
                    <span style={{ background:'#eff6ff', color:'#1d4ed8', fontWeight:700, fontSize:12, padding:'2px 10px', borderRadius:20 }}>{f.children_count?.[0]?.count??0}</span>
                  </td>
        ...
        {!loading && pages > 1 && (
          <div style={{ padding:'12px 16px', borderTop:'1px solid #f1f5f9', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            ...
            <div style={{ display:'flex', gap:4 }}>
              ...
              <button onClick={()=>setPg(p=>Math.min(pages,p+1))} disabled={pg===pages} style={{ padding:'5px 10px', border:'1px solid #e2e8f0', borderRadius:6, background:'#fff', cursor:'pointer', fontSize:12, color:'#64748b', opacity:pg===pages?.4:1 }}>Next →</button>
            </div>
          </div>
        )}
      </div>
...
// ─── FAMILY DETAIL PAGE ───────────────────────────────────────────────────────
function FamilyDetailPage({ family, allChildren, allDocs, onBack, onEditFamily, onAddChild, onEditChild, onDeleteChild, toast, onDocUploaded }) {
  ...
  return (
    <div style={{ padding:'22px 26px', maxWidth:1040, margin:'0 auto' }}>
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:18 }}>
        <button onClick={onBack} style={{ background:'#f1f5f9', border:'none', borderRadius:8, padding:'7px 14px', cursor:'pointer', fontSize:13, color:'#475569', fontWeight:650 }}>← Back</button>
        <div style={{ flex:1 }}/>
      ...
      <div style={{ background:'linear-gradient(135deg,#0c1f3f 0%,#1d3461 100%)', borderRadius:18, padding:'28px', marginBottom:20, display:'flex', alignItems:'flex-start', gap:24, flexWrap:'wrap' }}>
        ...
        <div style={{ flex:1, minWidth:200 }}>
          ...
          <div style={{ fontSize:13, color:'#93c5fd', marginBottom:14 }}>ID: {family.mother_id_number} · {family.family_code} · Roll {family.roll_number}</div>
          <div style={{ display:'flex', flexWrap:'wrap', gap:10 }}>
            {[{i:'📱',v:family.phone_number},{i:'📞',v:family.alternate_phone||'—'},{i:'📍',v:family.address},{i:'🏙',v:`${family.city}, ${family.district}`}].map((x,i)=>(
              <span key={i} style={{ fontSize:12, color:'#cbd5e1', display:'flex', alignItems:'center', gap:5 }}>{x.i} {x.v}</span>
            ))}
          </div>
        </div>
        <div style={{ display:'flex', gap:12, flexShrink:0 }}>
          {[{n:kids.length,l:'Children'},{n:famDocs.length,l:'Documents'}].map(s=>(
            <div key={s.l} style={{ background:'rgba(255,255,255,0.08)', borderRadius:12, padding:'12px 18px', textAlign:'center' }}>
              <div style={{ fontSize:24, fontWeight:800, color:'#93c5fd' }}>{s.n}</div>
      ...
      {tab==='overview' && (
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
          {[
            {l:'Family Code',v:family.family_code},{l:'Roll Number',v:family.roll_number},
            {l:'National ID',v:family.mother_id_number},{l:'Phone',v:family.phone_number},
            {l:'Alternate Phone',v:family.alternate_phone||'—'},{l:'City',v:family.city},
            {l:'District',v:family.district},{l:'Registered',v:family.created_at?.slice(0,10)},
          ].map(r=>(
              ...
              return (
                <div key={c.id} style={{ background:'#fff', borderRadius:14, border:'1px solid #e8f0fe', padding:'16px 20px' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:14 }}>
                    <Avatar name={c.child_name} photoUrl={c.child_photo_url} size={52} radius={12}/>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:700, fontSize:15, color:'#0f172a' }}>{c.child_name}</div>
                  ...
                  <div style={{ background:'#f8fafc', borderRadius:10, padding:'12px 14px' }}>
                    <div style={{ fontSize:11, fontWeight:700, color:'#64748b', textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:8 }}>📎 Documents</div>
                    <div style={{ display:'flex', gap:14, flexWrap:'wrap' }}>
                      {['Birth Certificate','School Certificate'].map(docType=>{
                        const existing = childDocs.find(d=>d.document_type===docType)
                        return (
                          <div key={docType} style={{ flex:1, minWidth:160 }}>
                            <div style={{ fontSize:11, color:'#64748b', marginBottom:6, fontWeight:600 }}>{docType}</div>
                            {existing
                              ? <div style={{ background:'#fff', border:'1px solid #e8f0fe', borderRadius:8, padding:'8px 12px', display:'flex', alignItems:'center', gap:8 }}>
                                  ...
                                  <span style={{ fontSize:11, color:'#1d4ed8', fontWeight:600, flex:1 }}>Uploaded ✓</span>
                                  <a href={existing.file_url} target="_blank" rel="noreferrer" style={{ background:'#eff6ff', border:'none', borderRadius:6, padding:'3px 9px', cursor:'pointer', fontSize:11, color:'#1d4ed8', fontWeight:650, textDecoration:'none' }}>View</a>
                                </div>
                              : <label style={{ border:'1.5px dashed #93c5fd', borderRadius:8, padding:'8px 12px', fontSize:11, color:'#1d4ed8', background:'#f8fbff', cursor:'pointer', textAlign:'center', display:'block', fontWeight:600 }}>
                                  + Upload
                            ...
                            {uploading===docType+c.id && <div style={{ fontSize:11, color:'#1d4ed8', marginTop:4 }}>⏳ Uploading…</div>}
                          </div>
                        )
                      })}
                    </div>
      ...
      {tab==='documents' && (
        <div>
          <div style={{ fontSize:13, color:'#64748b', marginBottom:16 }}>Mother's official documents — all images auto-compressed before upload</div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:14 }}>
            {[
              ...
              return (
                <div key={doc.type} style={{ background:'#fff', borderRadius:13, border:'1px solid #e8f0fe', padding:'18px', display:'flex', flexDirection:'column', gap:10 }}>
                  <div style={{ fontSize:28 }}>{doc.icon}</div>
                  <div style={{ fontWeight:700, fontSize:13, color:'#0f172a' }}>{doc.type}</div>
                  {existing
                    ? <div>
                        <div style={{ fontSize:11, color:'#10b981', fontWeight:700, marginBottom:6 }}>✓ Uploaded</div>
                        <a href={existing.file_url} target="_blank" rel="noreferrer" style={{ display:'block', background:'#eff6ff', border:'none', borderRadius:7, padding:'7px', cursor:'pointer', fontSize:12, color:'#1d4ed8', fontWeight:650, textAlign:'center', textDecoration:'none' }}>View / Download</a>
                      </div>
                    : <label style={{ border:'1.5px dashed #93c5fd', borderRadius:8, padding:'12px', textAlign:'center', cursor:'pointer', background:'#f8fbff', display:'block' }}>
                        <div style={{ fontSize:18, marginBottom:4 }}>📤</div>
                        <div style={{ fontSize:11, color:'#1d4ed8', fontWeight:600 }}>{uploading===doc.type ? '⏳ Uploading…' : 'Upload image or PDF'}</div>
                        <div style={{ fontSize:10, color:'#94a3b8', marginTop:1 }}>Auto-compressed</div>
          ...
          {famDocs.length>0 && (
            <div style={{ marginTop:20, background:'#fff', borderRadius:13, border:'1px solid #e8f0fe', overflow:'hidden' }}>
              <div style={{ padding:'14px 18px', borderBottom:'1px solid #f1f5f9', fontSize:13, fontWeight:700, color:'#0f172a' }}>All Uploaded Documents</div>
              {famDocs.map(d=>(
                <div key={d.id} style={{ padding:'12px 18px', borderBottom:'1px solid #f8fafc', display:'flex', alignItems:'center', gap:12 }}>
                  <span style={{ fontSize:20 }}>📄</span>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, fontWeight:650, color:'#1e293b' }}>{d.document_type}</div>
                    <div style={{ fontSize:11, color:'#94a3b8' }}>{d.file_name} · {d.file_size_kb}KB · {d.uploaded_at?.slice(0,10)}</div>
                  </div>
                  <a href={d.file_url} target="_blank" rel="noreferrer" style={{ background:'#eff6ff', border:'none', borderRadius:7, padding:'5px 12px', cursor:'pointer', fontSize:12, color:'#1d4ed8', fontWeight:650, textDecoration:'none' }}>View</a>
                </div>
              ))}
            </div>
          )}
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'flex-end', gap:10, marginTop:6 }}>
        ...
        <Btn onClick={()=>onSave(f, photoFile)} disabled={saving}>{saving?'Saving…':'💾 Save Family'}</Btn>
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      <div style={{ display:'flex', gap:20, alignItems:'flex-start', marginBottom:20, padding:'14px', background:'#f8fafc', borderRadius:12 }}>
        ...
        <div style={{ flex:1, fontSize:12, color:'#64748b', lineHeight:1.7, marginTop:20 }}>Clear photo of the child. Auto-compressed before saving.</div>
      </div>
      ...
      <div style={{ background:'#f8fafc', borderRadius:11, padding:'14px', marginBottom:14 }}>
        ...
        <div style={{ fontSize:11, color:'#94a3b8', marginTop:4 }}>📷 Photo or scan — auto-compressed up to 80% smaller.</div>
      </div>
      <div style={{ display:'flex', justifyContent:'flex-end', gap:10, marginTop:6 }}>
        <Btn onClick={onCancel} variant="secondary" disabled={saving}>Cancel</Btn>
        <Btn onClick={()=>onSave(f, photoFile, birthCertFile, schoolCertFile, familyId)} disabled={saving}>{saving?'Saving…':'💾 Save Child'}</Btn>
      </div>
    </div>
  )
}
            ...
            {[{l:'Active',v:byStatus.active,c:'#1d4ed8'},{l:'Inactive',v:byStatus.inactive,c:'#10b981'},{l:'Pending',v:byStatus.pending,c:'#f59e0b'}].map(x=>(
              <div key={x.l}>
                ...
                <div style={{ height:6, background:'#f1f5f9', borderRadius:3 }}>
                  <div style={{ height:'100%', borderRadius:3, background:x.c, width:`${(x.v/(families.length||1))*100}%` }}/>
                </div>
              </div>
            ))}
          </div>
          ...
          {Object.entries(byDistrict).map(([d,v])=>(
            <div key={d} style={{ marginBottom:11 }}>
              ...
              <div style={{ height:7, background:'#f1f5f9', borderRadius:4 }}>
                <div style={{ height:'100%', borderRadius:4, background:'#3b82f6', width:`${(v/(families.length||1))*100}%` }}/>
              </div>
            </div>
          ))}
        </div>
...
function LoginPage({ onLogin }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(160deg,#0c1f3f 0%,#1d3461 60%,#0c1f3f 100%)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ position:'absolute', inset:0, backgroundImage:'radial-gradient(circle at 25% 30%, rgba(29,78,216,0.2) 0%, transparent 55%)' }}/>
      <div style={{ background:'rgba(255,255,255,0.98)', borderRadius:20, padding:'40px 36px', width:'100%', maxWidth:400, boxShadow:'0 32px 80px rgba(0,0,0,0.4)', position:'relative' }}>
        <div style={{ textAlign:'center', marginBottom:28 }}>
          <div style={{ width:72, height:72, borderRadius:18, background:'#fff', border:'2px solid #e8f0fe', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 14px', overflow:'hidden', boxShadow:'0 4px 16px rgba(29,78,216,0.15)' }}>
            <img src="/logo.png" alt="Hidaya" style={{ width:64, height:64, objectFit:'contain' }}
              onError={e=>{ e.target.style.display='none'; e.target.nextSibling.style.display='flex' }}/>
            <div style={{ display:'none', width:64, height:64, alignItems:'center', justifyContent:'center', fontSize:32 }}>🕊</div>
          </div>
          <div style={{ fontSize:20, fontWeight:800, color:'#0c1f3f' }}>Hidaya Development</div>
          <div style={{ fontSize:13, color:'#64748b', marginTop:2 }}>Association — Case Management</div>
        </div>
        ...
        <div style={{ textAlign:'center', marginTop:14 }}>
          <button onClick={async()=>{ const e=prompt('Enter your email:'); if(e){ await supabase.auth.resetPasswordForEmail(e); alert('Password reset email sent!') }}} style={{ background:'none', border:'none', color:'#1d4ed8', fontSize:12, cursor:'pointer', fontWeight:600 }}>Forgot password?</button>
        </div>
      </div>
    </div>
  )
...
export default function App() {
  ...
  const saveFamily = async (form, photoFile) => {
    ...
    try {
      ...
      const payload = {
        family_code: form.family_code, roll_number: form.roll_number,
        mother_name: form.mother_name, mother_id_number: form.mother_id_number,
        phone_number: form.phone_number, alternate_phone: form.alternate_phone,
        address: form.address, city: form.city, district: form.district,
        notes: form.notes, status: form.status, mother_photo_url,
      }
  ...
  const saveChild = async (form, photoFile, birthCertFile, schoolCertFile, famId) => {
    ...
    try {
      ...
      const payload = {
        family_id: famId, child_name: form.child_name, gender: form.gender,
        date_of_birth: form.date_of_birth||null, grade: form.grade,
        school_name: form.school_name, medical_notes: form.medical_notes, child_photo_url,
      }
  ...
  if (authLoading) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#0c1f3f' }}>
      <div style={{ color:'#93c5fd', fontSize:16, fontWeight:600 }}>Loading Hidaya…</div>
    </div>
  )
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh', background:'#eef2f9', fontFamily:"'Segoe UI',system-ui,-apple-system,sans-serif" }}>
      ...
      <Sidebar page={page==='detail'?'families':page} setPage={p=>{setPage(p);setSelectedFamily(null)}} collapsed={collapsed} setCollapsed={setCollapsed} user={user} onSignOut={signOut}/>

      <main style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0 }}>
        <Header title={pageTitle} breadcrumb={breadcrumb}/>
        <div style={{ flex:1, overflowY:'auto' }}>
          {(page==='dashboard'||page==='families') && <DashboardPage families={families} children={children} loading={dataLoading} onView={f=>{setSelectedFamily(f);setPage('detail')}} onAdd={()=>{setEditingFamily(null);setShowFamilyForm(true)}}/>}
          {page==='detail' && selectedFamily && <FamilyDetailPage family={selectedFamily} allChildren={children} allDocs={docs} onBack={()=>setPage('dashboard')} onEditFamily={f=>{setEditingFamily(f);setShowFamilyForm(true)}} onAddChild={fid=>{setEditingChild(null);setChildFamilyId(fid);setShowChildForm(true)}} onEditChild={c=>{setEditingChild(c);setChildFamilyId(c.family_id);setShowChildForm(true)}} onDeleteChild={c=>setConfirmDelete({type:'child',item:c})} toast={toast} onDocUploaded={reloadDocs}/>}
          {page==='analytics' && <AnalyticsPage families={families} children={children}/>}
          {page==='settings' && <div style={{ padding:'60px', textAlign:'center', color:'#94a3b8' }}><div style={{ fontSize:40, marginBottom:10 }}>⚙️</div>Settings — user management coming soon.</div>}
        </div>
      </main>
      ...
      <Modal open={!!confirmDelete} onClose={()=>setConfirmDelete(null)} title="Confirm Deletion" width={400}>
        <div style={{ textAlign:'center', padding:'8px 0' }}>
          ...
          <div style={{ fontSize:13, color:'#64748b', marginBottom:22 }}><strong>{confirmDelete?.item?.mother_name||confirmDelete?.item?.child_name}</strong> will be permanently removed.</div>
